...
public class Slick2DJBox2DDebugDraw extends DebugDraw {
    ...
    public void drawPolygon(Vec2[] vertices, int vertexCount, Color3f color){
        if(vertexCount == 1){
                drawSegment(new Vec2(vertices[0].x * -1, vertices[0].y), new Vec2(vertices[0].x * -1, vertices[0].y), color);
                return;
        }
        
        for(int i=0; i<vertexCount-1; i+=1){
                drawSegment(new Vec2(vertices[i].x * -1, vertices[i].y), new Vec2(vertices[i+1].x * -1, vertices[i+1].y), color);
        }
        
        if(vertexCount > 2){
                drawSegment(new Vec2(vertices[vertexCount-1].x * -1, vertices[vertexCount-1].y), new Vec2(vertices[0].x * -1, vertices[0].y), color);
        }
    }
	...
	public void drawSolidPolygon(final Vec2[] vertices, final int vertexCount, final Color3f color) {
		...
		g.setColor(new Color(color.x,color.y,color.z, 0.5f));
